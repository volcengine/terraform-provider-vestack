...
func ResourceVestackCrRegistryState() *schema.Resource {
	return &schema.Resource{
		Create: resourceVestackCrRegistryStateCreate,
		Update: resourceVestackCrRegistryStateUpdate,
		Read:   resourceVestackCrRegistryStateRead,
		Delete: resourceVestackCrRegistryStateDelete,
...
func resourceVestackCrRegistryStateCreate(d *schema.ResourceData, meta interface{}) error {
	service := NewCrRegistryStateService(meta.(*bp.SdkClient))
	if err := bp.DefaultDispatcher().Create(service, d, ResourceVestackCrRegistryState()); err != nil {
		return fmt.Errorf("error on creating instance state %q, %w", d.Id(), err)
	}
	return resourceVestackCrRegistryStateRead(d, meta)
}

func resourceVestackCrRegistryStateUpdate(d *schema.ResourceData, meta interface{}) error {
	return fmt.Errorf("this resource does not allow update operation")
}

func resourceVestackCrRegistryStateRead(d *schema.ResourceData, meta interface{}) error {
	service := NewCrRegistryStateService(meta.(*bp.SdkClient))
	if err := bp.DefaultDispatcher().Read(service, d, ResourceVestackCrRegistryState()); err != nil {
		return fmt.Errorf("error on reading instance state %q, %w", d.Id(), err)
	}
	return nil
}

func resourceVestackCrRegistryStateDelete(d *schema.ResourceData, meta interface{}) error {
	service := NewCrRegistryStateService(meta.(*bp.SdkClient))
	if err := bp.DefaultDispatcher().Delete(service, d, ResourceVestackCrRegistryState()); err != nil {
