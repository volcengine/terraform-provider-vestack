...
```
$ terraform import vestack_iam_tag.default User:jonny:key1
```

*/

func ResourceVestackIamTag() *schema.Resource {
	tagsSchema := bp.TagsSchema()
	tagsSchema.ForceNew = true
	return &schema.Resource{
		Create: resourceVestackIamTagCreate,
		Read:   resourceVestackIamTagRead,
		Delete: resourceVestackIamTagDelete,
		Importer: &schema.ResourceImporter{
			State: func(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
				...
				service := NewIamTagService(meta.(*bp.SdkClient))
				...
				_ = d.Set("tags", schema.NewSet(bp.TagsHash, []interface{}{
...
func resourceVestackIamTagCreate(d *schema.ResourceData, meta interface{}) error {
	service := NewIamTagService(meta.(*bp.SdkClient))
	err := bp.DefaultDispatcher().Create(service, d, ResourceVestackIamTag())
	if err != nil {
...
func resourceVestackIamTagRead(d *schema.ResourceData, meta interface{}) error {
	service := NewIamTagService(meta.(*bp.SdkClient))
...
func resourceVestackIamTagDelete(d *schema.ResourceData, meta interface{}) error {
	service := NewIamTagService(meta.(*bp.SdkClient))
	err := bp.DefaultDispatcher().Delete(service, d, ResourceVestackIamTag())
	if err != nil {
