package certificate_test

import (
	"github.com/hashicorp/terraform-plugin-sdk/helper/resource"
	"github.com/volcengine/terraform-provider-vestack/vestack"
	"github.com/volcengine/terraform-provider-vestack/vestack/clb/certificate"
	"testing"
)

const testAccVestackCertificatesDatasourceConfig = `
resource "vestack_certificate" "foo" {
    certificate_name = "acc-test-certificate-${count.index}"
  	description = "acc-test-demo"
  	public_key = "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"
	private_key = "-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----"
  	project_name = "default"
  	tags  {
    	key = "k1"
    	value = "v1"
  	}
	count = 3
}

data "vestack_certificates" "foo"{
    ids = vestack_certificate.foo[*].id
}
`

func TestAccVestackCertificatesDatasource_Basic(t *testing.T) {
	resourceName := "data.vestack_certificates.foo"

	acc := &vestack.AccTestResource{
		ResourceId: resourceName,
		Svc:        &certificate.VestackCertificateService{},
	}

	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			vestack.AccTestPreCheck(t)
		},
		Providers: vestack.GetTestAccProviders(),
		Steps: []resource.TestStep{
			{
				Config: testAccVestackCertificatesDatasourceConfig,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(acc.ResourceId, "certificates.#", "3"),
				),
			},
		},
	})
}
