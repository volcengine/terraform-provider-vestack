...
```
$ terraform import vestack_vpc_prefix_list.default resource_id
```

*/

func ResourceVestackVpcPrefixList() *schema.Resource {
	resource := &schema.Resource{
		Create: resourceVestackVpcPrefixListCreate,
		Read:   resourceVestackVpcPrefixListRead,
		Update: resourceVestackVpcPrefixListUpdate,
		Delete: resourceVestackVpcPrefixListDelete,
...
func resourceVestackVpcPrefixListCreate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewVpcPrefixListService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Create(service, d, ResourceVestackVpcPrefixList())
	if err != nil {
		return fmt.Errorf("error on creating vpc_prefix_list %q, %s", d.Id(), err)
	}
	return resourceVestackVpcPrefixListRead(d, meta)
}

func resourceVestackVpcPrefixListRead(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewVpcPrefixListService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Read(service, d, ResourceVestackVpcPrefixList())
	if err != nil {
...
func resourceVestackVpcPrefixListUpdate(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewVpcPrefixListService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Update(service, d, ResourceVestackVpcPrefixList())
	if err != nil {
...
func resourceVestackVpcPrefixListDelete(d *schema.ResourceData, meta interface{}) (err error) {
	service := NewVpcPrefixListService(meta.(*bp.SdkClient))
	err = service.Dispatcher.Delete(service, d, ResourceVestackVpcPrefixList())
	if err != nil {
