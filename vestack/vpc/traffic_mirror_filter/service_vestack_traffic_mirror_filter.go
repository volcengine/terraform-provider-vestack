...
func (s *VestackTrafficMirrorFilterService) ReadResources(m map[string]interface{}) (data []interface{}, err error) {
	var (
...
func (s *VestackTrafficMirrorFilterService) ReadResource(resourceData *schema.ResourceData, id string) (data map[string]interface{}, err error) {
	var (
		results []interface{}
		ok      bool
	)
	...
	if len(data) == 0 {
		return data, fmt.Errorf("traffic_mirror_filter %s not exist ", id)
	}
	return data, err
}

func (s *VestackTrafficMirrorFilterService) RefreshResourceState(resourceData *schema.ResourceData, target []string, timeout time.Duration, id string) *resource.StateChangeConf {
	return &resource.StateChangeConf{
		...
		Refresh: func() (result interface{}, state string, err error) {
			...
			for _, v := range failStates {
				if v == status.(string) {
					return nil, "", fmt.Errorf("traffic_mirror_filter status error, status: %s", status.(string))
				}
			}
...
func (s *VestackTrafficMirrorFilterService) ReadResourceId(id string) string {
	return id
}
