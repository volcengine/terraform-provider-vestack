...
subcategory: "VKE"
layout: "vestack"
page_title: "Vestack: vestack_vke_clusters"
sidebar_current: "docs-vestack-datasource-vke_clusters"
description: |-
  Use this data source to query detailed information of vke clusters
---
# vestack_vke_clusters
Use this data source to query detailed information of vke clusters
## Example Usage
```hcl
...
* `clusters` - The collection of VkeCluster query.
    ...
    * `pods_config` - The config of the pods.
        * `calico_config` - Calico network configuration.
            * `bgp_config` - Configuration information of BGP mode under Calico. Only supported in Onpremise cluster & CalicoBgp mode.
                * `as_number` - Value range [64512, 65534].
                * `external_route_reflector_enabled` - Whether to enable external RoutReflector, optional values true | false: - true: Enable external RoutReflector instead of using Master node as RoutReflector. - false: Use Master node as RoutReflector. Default value: false.
                * `mode` - BGP mode, optional values are: FullMesh | RouteReflectors: - FullMesh: all nodes serve as RR peers. - RouteReflectors: The Master node as the RR contains the routing information of all nodes, and the node only has routes pointing to the RR node.
                * `route_reflector_peer_points` - Pod CIDR for the Flannel container network.
                    * `as_number` - If ExternalRouteReflectorEnabled=true, this parameter is optional, otherwise this parameter cannot be empty [64512, 65534].
                    * `ip_address` - IP address of RouteReflector.
                    * `port` - Port of RouteReflector.
            * `max_pods_per_node` - The maximum number of single-node Pod instances for a Flannel container network, the value can be `16` or `32` or `64` or `128` or `256`, default value is `64`.
