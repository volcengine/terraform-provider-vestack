...
resource "vestack_ebs_snapshot_group" "foo" {
  volume_ids   = [vestack_ecs_instance.foo.system_volume_id, vestack_volume.foo.id]
  instance_id  = vestack_ecs_instance.foo.id
  ...
  depends_on = [vestack_volume_attach.foo]
}

data "vestack_ebs_snapshot_groups" "foo" {
  ids = [vestack_ebs_snapshot_group.foo.id]
}
